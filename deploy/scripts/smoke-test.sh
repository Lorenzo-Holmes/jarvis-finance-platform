...
assert_agent_stream() {
  ...
  run_id="$(AGENT_BODY_FILE="$body_file" python3 -c 'import json,os,pathlib; lines=pathlib.Path(os.environ["AGENT_BODY_FILE"]).read_text().splitlines(); events=[json.loads(x[5:].strip()) for x in lines if x.startswith("data:")]; assert events, "no Agent SSE data"; ids={e.get("runId") for e in events}; assert len(ids)==1 and next(iter(ids)), ids; calls=[e for e in events if e.get("type")=="tool_call"]; assert calls, events; by_step={}; [by_step.setdefault(e.get("stepId"),[]).append(e.get("type")) for e in events if e.get("stepId")]; for_call=[(e,by_step.get(e.get("stepId"),[])) for e in calls if e.get("status")=="running"]; assert for_call, events; assert all(all(t in types for t in ("step_started","tool_call","tool_result","step_completed")) and types.index("step_started") < types.index("tool_call") < types.index("tool_result") < types.index("step_completed") for _,types in for_call), by_step; assert any(e.get("type") in {"run_completed","run_failed","run_cancelled"} for e in events), events[-1]; seq=[int(e.get("sequence",0)) for e in events]; assert seq==sorted(seq), seq; print(next(iter(ids)))')"
  echo "OK  $label"
