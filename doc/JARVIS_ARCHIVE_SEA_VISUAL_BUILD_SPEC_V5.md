...
- [x] 桌面 Browse 镜头按目标截图的透视斜率继续纠偏：实际 Browse 距离保持 `100`，水平环绕角由约 `59°` 调整到约 `72.5°`，俯角由约 `9.5°` 下调到约 `8.5°`；FOV 仍按 140 world-unit reference distance 计算，让档案长边投影更接近目标图约 29° 的斜率，同时保持低视线和档案正面占屏。
