...
- [x] 不通过继续扩大静态网格来掩盖边界；仍复用有限的轻量档案池。
- [x] 物理池固定为 `9 × 25` 个轻量程序化档案；用户看到的是周期映射后的虚拟坐标，不是物理池坐标。
...
- [x] 周期映射后的 9 个 lane / 25 个 row 在任意远距离 center 下必须连续、无重复、无缺口。
- [x] P0 当前为 `54 / 54` 通过；production build 已通过。
