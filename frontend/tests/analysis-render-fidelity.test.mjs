...
test('archive field composition V5 matches the wide-card long-lens archive reference while keeping anonymous backgrounds', () => {
  ...
  assert.match(scene, /const distance = 140/)
  assert.match(scene, /const yaw = 59 \* Math\.PI \/ 180/)
  assert.match(scene, /const elevation = 19 \* Math\.PI \/ 180/)
  ...
  assert.match(scene, /focusArchiveComposer\(composerBundle, camera\.position\.distanceTo\(cameraAim\)\)/)
