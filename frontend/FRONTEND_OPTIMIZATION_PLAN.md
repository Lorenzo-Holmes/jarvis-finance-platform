...
- [x] SMA / EMA / 支撑阻力摘要已通过统一指标接口接入行情首页 Rail。
...
- [x] 净值主图和近期交易表重新组织。
- [x] 最大回撤独立曲线、Sharpe、胜率等已由后端返回并接入前端。

当前状态：高级绩效指标已接入；仍需持续用真实业务数据观察结果质量。
...
- [x] 运维增加统一总体状态和立即检查入口。
- [x] GitHub username / OAuth 来源已由后端 Admin DTO 返回并接入。
- [x] 审计记录已由后端查询 API 返回并接入前端；生产管理员账号的端到端验收另行记录。
